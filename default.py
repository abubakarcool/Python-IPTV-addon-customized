...
if sys.argv[1] == 'SetPassword':
    ...
    if exists == False:
        ...
        ps = dialog.numeric(0, 'Enter current password:')
        if ps == p_file_b64_decode:
            ps2 = dialog.numeric(0, 'Enter the new password:')
            if ps2 != '':
                ps2_b64 = base64.b64encode(ps2.encode()).decode('utf-8')
                p_file = open(arquivo,'w')
                p_file.write(ps2_b64)
                p_file.close()
                xbmcgui.Dialog().ok('[B][COLOR white]WARNING[/COLOR][/B]','Password has been changed successfully!')
            else:
                xbmcgui.Dialog().ok('[B][COLOR white]WARNING[/COLOR][/B]','Unable to change password!')
        else:
            xbmcgui.Dialog().ok('[B][COLOR white]WARNING[/COLOR][/B]','Invalid password!, if not changed use the default password')
    else:
        p_file = open(arquivo,'r+')
        p_file_read = p_file.read()
        p_file_b64_decode = base64.b64decode(p_file_read).decode('utf-8')
...
def browser(url):
    ...
    try:
        ...
        if hasattr(e, 'code'):
            xbmc.executebuiltin("Notification(Failed, error code - "+str(e.code)+",10000,"+__icon__+")")    
        elif hasattr(e, 'reason'):
            xbmc.executebuiltin("Notification(Failed, reason - "+str(e.reason)+",10000,"+__icon__+")")
        content = ''
    return content        
...
def status():
    ...
    if status > '' and status == 'Active':
        ...
        status_result = 'Expired or Doesnot Exist'
    if str(exp_date) > '':
    ...
    if max_connections > '':
        ...
        limite_conexao = ''
    msg = 'Status: [COLOR yellow]%s[/COLOR]\nExpires in: [COLOR yellow]%s[/COLOR]\nStatement: [COLOR yellow]%s[/COLOR]\nCreated in: [COLOR yellow]%s[/COLOR]\nAllowed connections: [COLOR yellow]%s[/COLOR]'%(status_result,str(expires),vip_trial,str(created_time),str(limite_conexao))
    xbmcgui.Dialog().textviewer('Premium account:', msg)        
...
def home():
    addDir('[B]To enter[/B]','',2,'','','')
    addDir('[B]Status[/B]','',5,'','','')
    addDir('[B]Settings[/B]','',1,'','','')    
    SetView('WideList')
...
def main():
    ...
    if mode==None:
        ...
        if re.search("Adult",name,re.IGNORECASE) or re.search("A Casa das Brasileirinhas",name,re.IGNORECASE):
            ...
            p_file_b64_decode = base64.b64decode(p_file_read).decode('utf-8')
            dialog = xbmcgui.Dialog()
            ps = dialog.numeric(0, 'Enter the current password:')
            if ps == p_file_b64_decode:
                get_m3u8_2(name,url)
            else:
                xbmcgui.Dialog().ok('[B][COLOR white]WARNING[/COLOR][/B]','Invalid password!, if not changed, use the default password')
